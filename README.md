...
[![Status](https://img.shields.io/badge/status-v0.6.6--alpha-blue.svg)](https://github.com/metahub-tech/agent-fleet/releases/tag/v0.6.6-alpha)
...
>
> ```bash
> uvx --from "git+https://github.com/metahub-tech/agent-fleet@v0.6.6-alpha#subdirectory=cli" agent-fleet setup
> ```
...
| **macOS legacy-plist migration cleanup** | **`0.6.6-alpha`** | ✅ **Released** (setup-macos.sh / setup-android.sh now detect + unload + remove pre-v0.6.0 `cc.metahub.macbox-gui` and `cc.metahub.android-gui` plists, plus pkill orphaned `*_mcp.py` processes — no more port-squatting after rename) |
...
或，如果已经有 uv（v0.6.6-alpha 阶段从 git 拉，未上 PyPI）：
