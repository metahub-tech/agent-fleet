...
|---|---|---|
| Мост Windows 10/11 | `0.2.0` | ✅ Выпущен (win-device, 40 инструментов, streamable-http) |
| Мост macOS 12+ | `0.3.0` | ✅ Выпущен (mac-device, launchd, 41 инструмент, процесс выдачи GUI-разрешений) |
...
| Координация между устройствами | `0.10.0` | 🔭 В будущем |
...
Интерфейс инструментов остаётся семантически согласованным на всех платформах (`take_screenshot` / `tap` / `launch_app` / …); смена устройства — это просто замена URL в `~/.claude.json`.
