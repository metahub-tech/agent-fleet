...
def setup_env(ctx: InstallContext, role_id: str) -> dict[str, str]:
    ...
    pause point. Platform-specific options collected via ManifestInstaller.collect_options()
    are injected generically from ctx.platform_options.
    Built on top of os.environ so PATH and friends survive.
    """
    env = {**os.environ, "ATB_WIZARD_MANAGED": "1"}
    for k, v in ctx.platform_options.items():
        env[k] = v
    return env
