...
iOS is sandboxed. There is no `run_shell`. If you need to run shell commands on the macOS host itself, use mac-device's `run_shell` tool (separate MCP server, port 8767).

| Need | Tool | Server |
|---|---|---|
| On-device shell | **NOT AVAILABLE** (iOS sandbox) | — |
| Host shell (macmini) | `run_shell(script=...)` | mac-device (port 8767) |

### App lifecycle

```
list_apps(filter_substring="safari", only_user=False)  # find installed apps
launch_app(target="com.apple.MobileSafari")            # launch via WDA
current_app()                                           # what's in front
terminate_app(target="com.apple.MobileSafari")         # force-terminate via WDA
activate_app(bundle_id="com.example.MyApp")            # bring to foreground (must be running)
install_app(path="/path/on/machost/foo.ipa")           # install .ipa on device
uninstall_app(target="com.example.MyApp")              # uninstall
```

`launch_app` uses WDA `/wda/apps/launch`; `terminate_app` uses `/wda/apps/terminate`. Both require WDA to be running.
...
| App lifecycle | `list_apps` / `install_app` / `uninstall_app` / `launch_app` / `terminate_app` / `activate_app` / `current_app` | install = pymobiledevice3; launch/terminate/activate/current = WDA |
...
| `type_text` doesn't land | Some apps use custom text rendering that bypasses UIKit key injection. Workaround: tap a standard text field first, then type. Or use clipboard paste via run_shell + pbcopy on the host. |
...
- **No `run_shell`** — iOS is sandboxed. For host-side shell use mac-device's `run_shell`.
...
- "I'll use `run_shell`" → no equivalent on iOS; use mac-device `run_shell` for host-side shell
