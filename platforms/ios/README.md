# iOS Platform Bridge — v0.8.0-alpha

> 状态：已实现（v0.8.0-alpha）。WDA + pymobiledevice3 实现，无 Appium / XCTest 直接依赖。支持多设备同时挂载（别名映射 + per-device holder 锁）。首批验证机型：iPad（iOS 26.2.1）。
