...
def find_elements(
    ...
    exposed via AX — for web / no-AX-tree apps (Electron a11y-off, Java Swing,
    Flutter), fall back to vision_locate / vision_tap (像素级定位, if the vision
    capability is enabled) rather than guessing coords off a screenshot.
    """
...
# change). The registry adds the list_capabilities() discovery tool and will
# register optional capability modules (browser, ...) here when enabled in
# config. Static registration at import → the client defers tool schemas
# (design §1), so this costs no upfront tokens.
_PLATFORM_DIR = Path(__file__).resolve().parent.parent      # platforms/macos
...
# 注:本模块也被 server 测试以 `import mac_device_mcp` 形式导入(此时 sys.argv 是 pytest 的),
# 故 parse 失败(SystemExit/未知参数)退回纯默认端口,保证导入期 floor check 不被打断。
try:
    _server_args = parse_server_args(prog="agent-fleet-mac", default_port=8767)
except SystemExit:
    _server_args = parse_server_args(prog="agent-fleet-mac", default_port=8767, argv=[])
...
def main() -> None:
    ...
    _server_runtime.serve(mcp, prog="agent-fleet-mac", default_port=8767, args=_server_args)
