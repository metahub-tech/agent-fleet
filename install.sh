...
#
# Env-var overrides:
#   AGENT_FLEET_VERSION      default v0.6.11-alpha
...
AGENT_FLEET_VERSION="${AGENT_FLEET_VERSION:-v0.6.11-alpha}"
...
ERROR: agent-fleet's installer is interactive but stdin is not a TTY.
  ...
  Or, since v0.6.11-alpha already cloned the repo for you in a prior attempt:

      cd ~/agent-fleet
      uvx --from "git+https://github.com/metahub-tech/agent-fleet@v0.6.11-alpha#subdirectory=cli" agent-fleet setup
