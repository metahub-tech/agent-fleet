...
AGENT_FLEET_VERSION="${AGENT_FLEET_VERSION:-v0.6.14-alpha}"
...
ERROR: agent-fleet's installer is interactive but stdin is not a TTY.
  ...
  Or, since v0.6.14-alpha already cloned the repo for you in a prior attempt:

      cd ~/agent-fleet
      uvx --from "git+https://github.com/metahub-tech/agent-fleet@v0.6.14-alpha#subdirectory=cli" agent-fleet setup
